    ...
    return (
      <div className="bg-white p-3 border border-slate-200 shadow-lg rounded-lg">
        ...
        <p className="text-sm font-bold text-slate-900">
          {payload[0].value.toLocaleString()} <span className="text-[10px] font-normal text-slate-400">{unit}</span>
        </p>
      </div>
    );
  }
...
const RiskMetricBox = ({ label, subtitle, value, unit, description, type }: any) => {
  ...
  return (
    <div className={`p-6 rounded-2xl border ${themeClasses} backdrop-blur-sm flex flex-col justify-between h-full group transition-all hover:scale-[1.02] hover:shadow-lg`}>
      <div>
        <div className="flex justify-between items-start mb-4">
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.1em] mb-1">{label}</p>
            <p className="text-[10px] font-bold text-slate-500 uppercase opacity-70">{subtitle}</p>
          </div>
          <div className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-tighter ${isNegative ? 'bg-red-500/10 text-red-500' : 'bg-emerald-500/10 text-emerald-500'}`}>
            {type}
          </div>
        </div>
        <div className="flex items-baseline gap-2 mb-4">
          <span className={`text-4xl font-black ${isNegative ? 'text-red-400' : 'text-emerald-400'}`}>
            {value}
          </span>
          <span className="text-xs font-bold text-slate-500 uppercase">{unit}</span>
        </div>
      </div>
      <p className="text-xs text-slate-400 leading-relaxed font-medium pb-2 border-t border-white/10 pt-4 mb-0 italic">
        {description}
      </p>
    </div>
  );
    ...
    <div className="mb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
                ...
                <div key={idx} className="p-5 rounded-2xl bg-slate-50 border border-slate-100 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-lg font-black text-slate-800">{t.price} <span className="text-xs text-slate-400">{contract.unit}</span></p>
                    </div>
          ...
          <div className="glass-card p-8 flex-1 bg-slate-900 border-none">
            <h3 className="text-sm font-bold text-white mb-6 flex items-center gap-2">
              <AlertTriangle size={20} className="text-amber-400" /> 战略风险指标回测
            </h3>
...
const ExposureView = () => {
  const pieData = [
    { name: '一口价采购', value: EXPOSURE_DATA.fixedPrice, color: '#0f172a', percentage: 87.5 },
    { name: '风险敞口', value: EXPOSURE_DATA.riskExposure, color: '#ef4444', percentage: 12.5 }
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="glass-card p-10 flex flex-col items-center justify-center min-h-[480px] hover:shadow-xl transition-all border-slate-200">
           <div className="w-full flex justify-between items-center mb-6">
              ...
              <div className="text-right">
                 <p className="text-3xl font-black text-red-600">12.5%</p>
           ...
           <div className="w-full h-[300px] relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                 ...
                 <p className="text-3xl font-black text-slate-900">{(EXPOSURE_DATA.fixedPrice + EXPOSURE_DATA.riskExposure).toLocaleString()}</p>
                 <p className="text-[10px] font-bold text-slate-400">万元</p>
              </div>
           </div>
              ...
              {pieData.map((item, idx) => (
                 <div key={idx} className="flex items-center gap-3 p-4 rounded-2xl bg-white border border-slate-100 shadow-sm hover:border-slate-300 transition-colors">
                    ...
                    <div>
                       ...
                       <p className="text-base font-black text-slate-800">{item.value.toLocaleString()} <span className="text-[10px] font-bold text-slate-400 ml-1">万</span></p>
                    </div>
                 </div>
              ))}
           </div>
        ...
        {/* Chart 2: Structure */}
        <div className="glass-card p-10 flex flex-col items-center justify-center min-h-[480px] hover:shadow-xl transition-all border-blue-100">
           <div className="w-full flex justify-between items-center mb-6">
              <h4 className="text-xl font-black text-slate-800 flex flex-col">
              ...
              <div className="text-right">
                 <p className="text-3xl font-black text-blue-600">4,913</p>
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">敞口总计 (万元)</p>
              </div>
           </div>
           <div className="w-full h-[300px] relative">
              <ResponsiveContainer width="100%" height="100%">
           ...
           <div className="w-full grid grid-cols-2 gap-3 mt-8">
              {outerData.map((item, idx) => (
                 <div key={idx} className="flex items-center gap-3 p-4 rounded-xl bg-blue-50/50 border border-blue-100/50 hover:bg-blue-50 hover:border-blue-200 transition-all">
                    ...
                    <div className="flex-1 flex justify-between items-center">
                       ...
                       <div className="text-right">
                          ...
                          <p className="text-[10px] font-bold text-slate-400">{item.amount.toLocaleString()}万</p>
                       </div>
                    </div>
                 </div>
              ))}
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-slate-200 pb-10 gap-8">
          ...
          <div className="flex gap-6">
             <div className="px-6 py-4 bg-white rounded-2xl shadow-md border border-slate-100 text-right min-w-[180px]">
                ...
                <p className="text-3xl font-black text-slate-900">{SUMMARY_STATS.totalPlanned.toLocaleString()} <span className="text-sm font-bold text-slate-400 uppercase">万元</span></p>
             </div>
             <div className="px-6 py-4 bg-blue-50 rounded-2xl shadow-md border border-blue-100 text-right min-w-[180px]">
                ...
                <p className="text-3xl font-black text-blue-600">{SUMMARY_STATS.closedExposure.toLocaleString()} <span className="text-sm font-bold text-blue-400 uppercase">万元</span></p>
             </div>
             <div className="px-6 py-4 bg-red-50 rounded-2xl shadow-md border border-red-100 text-right min-w-[180px]">
                ...
                <p className="text-3xl font-black text-red-600">{SUMMARY_STATS.currentExposure.toLocaleString()} <span className="text-sm font-bold text-red-400 uppercase">万元</span></p>
             </div>
          </div>
        </div>
      </header>
